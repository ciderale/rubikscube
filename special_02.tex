...
\begin{scope}[shift={(+\theradius+1.5,+4.5)}]
  \draw (0,0) coordinate(X);
  \draw[->,very thick] (X) -- ++(2,0) node[pos=0.8,arrowlabel]{1} coordinate(X);
  \draw[->,very thick] (X) -- ++(0,2) node[pos=0.8,arrowlabel]{2} coordinate(X);
  \draw[->,very thick] (X) -- ++(-2,0) node[pos=0.8,arrowlabel]{3} coordinate(X);
  \draw[->,very thick] (X) -- ++(0,-1.8) node[pos=0.8,arrowlabel]{4} coordinate(X);
  \draw (1,1) node{\bf +1};
\end{scope}
...
\begin{scope}[shift={(+\theradius+1.5,-4.5)}]
  ...
  \draw[->,very thick] (X) -- ++(0,2) node[pos=0.8,arrowlabel]{2} coordinate(X);
  \draw[->,very thick] (X) -- ++(2,0) node[pos=0.8,arrowlabel]{1} coordinate(X);
  \draw[->,very thick] (X) -- ++(0,-2) node[pos=0.8,arrowlabel]{4} coordinate(X);
  \draw[->,very thick] (X) -- ++(-1.8,0) node[pos=0.8,arrowlabel]{3} coordinate(X);
  \draw (1,1) node{\bf -1};
\end{scope}

\draw[very thick,->] (+5.5,+4.3) to[out=-130,in=130] ++(0.2,-1.4) node[above=10]{\bf +1};
\draw[very thick,->] (+5.8,-4.5) to[out=120,in=-120] ++(0,6) node[below=10]{\bf -1};
